...
def draw_menu(display):
    ...
    menu_x + button_width + 10, menu_y, button_width, button_height))  # 10 is the space between the buttons
    ...
    display.blit(quit_text, (
    menu_x + button_width / 2 - quit_text.get_width() / 2, menu_y + button_height / 2 - quit_text.get_height() / 2))
    display.blit(reset_text, (menu_x + button_width + button_width / 2 - reset_text.get_width() / 2 + 10,
                              menu_y + button_height / 2 - reset_text.get_height() / 2))  # 10 is the space between the buttons
...
def draw_line_animation(display, coordinates, progress):
    ...
    start_x = start_x * cell_size + cell_size // 2
    start_y = (rows - 1 - start_y) * cell_size + cell_size // 2 + offset_y
    end_x = end_x * cell_size + cell_size // 2
    end_y = (rows - 1 - end_y) * cell_size + cell_size // 2 + offset_y
    # Calculate the intermediate coordinates based on the progress
    intermediate_x = start_x + progress * (end_x - start_x)
    intermediate_y = start_y + progress * (end_y - start_y)
    pygame.draw.line(display, (255, 255, 255), (start_x, start_y), (intermediate_x, intermediate_y), 10)
...
while running:
    ...
    if game_over:
        # Draw the line animation before the overlay
        if win_coordinates is not None and not line_animation_done:
        ...
        screen.blit(winner_text, (350 - winner_text.get_width() // 2,
                                  300 - winner_text.get_height() // 2))  # Draw the text on the screen, not the overlay
