...
def draw_grid(display, board):
    for i in range(6):  # There are 6 rows in the Connect Four board
        for j in range(7):  # There are 7 columns in the Connect Four board
            # pygame.draw.rect(display, (255, 255, 255), (j * cell_size, i * cell_size, cell_size, cell_size), 1)  # Commented out
            pygame.draw.circle(display, (255, 255, 255), (j * cell_size + cell_size // 2, i * cell_size + cell_size // 2), cell_size // 2 - 5)
            if board[i][j] != ' ':
                color = (255, 0, 0) if board[i][j] == 'X' else (255, 255, 0)  # Change the color of the second player's tokens to yellow
                pygame.draw.circle(display, color, (j * cell_size + cell_size // 2, i * cell_size + cell_size // 2), cell_size // 2 - 5)
game_over = False
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            game.switch_player()  # Switch the player immediately after a piece is dropped
            win_coordinates = game.check_win()
            if win_coordinates:
                pygame.display.set_caption(f"Player {game.current_player} wins!")  # Changed from 'X' or 'O' to '1' or '2'
                game_over = True
    screen.fill("blue")
    draw_grid(screen, game.board)  # Draw the grid on each frame
    if game_over:
        start_pos = (
